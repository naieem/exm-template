...
if ( ! function_exists( 'theme_client_woocommerce_product_columns_wrapper' ) ) {
	...
	function theme_client_woocommerce_product_columns_wrapper() {
		...
		echo '<div class="columns-' . absint( $columns ) . ' shop-products-wrapper">';
	}
}
...
function theme_client_ajax_woo_cat_filter(){
	...
	$args = [
		...
		'offset'	=> intval($_POST['currentCount'])
	];
...
add_filter( 'woocommerce_get_stock_html', function($html, $product){
			...
			switch($key){
				case 'bl': 
					...
					case 'ed': 
						...
						//$total += round(intval($stock),0);
						//if ($prod->get_stock_quantity() == 0 || $prod->get_stock_quantity() > $total) {
							//	wc_update_product_stock( $prod, $total);
							//}
							?>
				<li>
	...
	$boxcadUrl = get_field('boxcad_url',$product->is_type('variation')?$product->get_parent_id():$product->get_id());

	if(!empty($boxcadUrl)){ ?>
		<a rel="noopener" target="_blank" id="boxcadUrl" class="button customize" href="<?php echo $boxcadUrl;?>"><?php echo __('Customize on BOXCAD','theme-client')?></a>
	<?php }
...
add_filter('woocommerce_available_variation',function($variation){
	...
	if($key_included && $key_number){
		...
		$html_add .= '<div class="product_info"><h3 class="">'.__('Key number','theme-client').': '.$key_number.'</h3></div><input type="text" value="dhh">';
